...
| `bars` | GET | `name` | WHERE ONE COMMAND LIVES, AND WHICH COPY ANSWERS. `command` reports one word - `currently disabled` - and a command is offered in several places at once: Reset is on the menu bar, on two toolbars and on three context menus. This surface hides every toolbar the editor came with, so that refusal used to be the opinion of whichever hidden copy the walk met first. `places` is every control carrying it with its bar, that bar's `barVisible`, and its own `enabled`; `enabledCount` of zero is the editor genuinely refusing. `mode` is the ACTIVE project's, read live (0 run, 1 break, 2 design), beside `publishedMode` - the word the page was last told - because the poll that publishes swallows its failures and the two can drift. `modeError` names why a live read failed. `name` takes a command name or a bare id |
