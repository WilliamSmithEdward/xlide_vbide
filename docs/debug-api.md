...
| `changes` | GET | `action=text\|diff\|snapshot\|accept`, `project`, `round`, `module`, `which=before\|after`, `label`, `limit`, `by` | the change log: what happened to a project's module code, by whom, in rounds. No action lists the rounds newest first, the one still running at the front, each with who wrote it, what it was called if anyone said, and one entry per module carrying `+added -removed` worked out from the texts the log kept. `text` answers what a module held before or after a round, `diff` lines the two up as the import dialog's own comparison does. `snapshot&label=` ends the round that is running and names it - it costs nothing, because a round is a divider rather than a copy - and `accept` draws the accepted line without destroying anything above or below it. **READ-ONLY: there is no revert here and there is not going to be one.** Putting text back is a write, and this door already has a hardened one at `module`: read the text you want and write it, and the result lands in this log like any other write, as visible and as reversible as what it undid. A round is ended by a snapshot, by the writing hand changing, by a workbook save, or by five minutes' silence. What it records is MODULE CODE and nothing else - not references, not project properties, not a form's design, and not edits made directly in the VBE - which it says in its own `covers` field rather than leaving a reader to assume. `by=` on any route names the caller and is what makes the log worth reading once two agents are working; without it a write through this door is recorded as `unattributed`, because it is certainly not the developer typing |
