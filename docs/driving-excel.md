...
| `settings` | `settings()` / `settings({...})` | read them, or change one without restating the rest. `designerSnap` is `grid`, `objects` or `off` - the designer's snapping, one or the other |
