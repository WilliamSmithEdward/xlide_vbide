...
| `scm` | `scm({action, project, module, ref, message, folder, name, email, url, remote, limit, by, modules, timeoutMs})` | source control, one brain with the Source Control pane. Bare it answers the status: `state`, the folder and the repository root, the branch, `rows` (the LIVE project against the branch head), `outside` (folder files that differ from the live text), the branches, the last commit and a `suggestedMessage` from the change-log rounds since the last commit. `settings&folder=` remembers the folder, `init` makes a repository there, `identity` names the committer, `remote` attaches origin from `url` or re-points it, `commit` saves, exports and commits the `modules` named (none means every changed row) with `git commit --only`, `import` writes Folder rows into the project, `log`/`diff`/`show` read history, `open` opens a past version in a read-only tab, `restore` writes one back as a change-log round, `blame` maps the committed lines onto the editor's, `checkout` refuses over a dirty workbook and imports once it is clean, `fetch`/`pull`/`push`/`abort` run git and answer its words, a first push setting the upstream itself. Acting verbs ride as POST and answer the status after acting; commit and import carry it under `status`. Needs git.exe on the machine, and answers `state: noGit` without it |
...
| `project` | `project(project)` | what the VBA project CONTAINS: components, kinds, line counts, panes, and for a CLASS its `predeclaredId`. That last one is `Attribute VB_PredeclaredId`, which the object model has no property for and a code pane never shows - it is read out of the SAVED package on disk, and it is what decides whether `Ticket.ChangeTest` compiles or reports. `true`/`false`/`null`, and null is a class the saved file cannot answer for (one added since the last save), which is the state that keeps the analyzer silent. `references` is the project's type libraries - `name`, `guid`, `broken` - which is the OTHER fact a finding depends on and the text does not carry: `Dim wd As Word.Application` is correct in a project that references Word and `missing-library-reference` in one that does not |
| `reference` | `addReference(module, line, {project, column})` | gives the project the library a `missing-library-reference` finding names, by applying the quick fix on that finding. Adding a reference is not a text edit - it writes a record into the project - so it travels as a command the host performs, and this is the same path the lightbulb takes. Answers `{ok, library, guid, added}`. The findings move on their own afterwards: nothing in any module's text changed, which is exactly why both sameness gates compare the reference set |
...
- **A form's pair is a CREATE here, by decision.** The shared planner refuses to create a
  ...
  mechanism that would make the override unnecessary). NOT IN ACCESS: its VBE carries no
  MSForms - Access VBA has no UserForms at all - so the promotion stands down there and the
  planner's refusal is the truth. The same rule gates every create surface: the page offers
  no "New UserForm" in Access, and `component?action=add&kind=form` refuses with the reason
  rather than relaying the COM failure the VBE would answer.
