...
| Performance | measuring | 20 items. `dev.ps1` repackaging the engine every run is done. C7/C8 (host-thread COM reads on tab switch and analysis pass) are now INSTRUMENTED - `perf().hostReadMs`, the counter the findings said did not exist - and MEASURED on the perf fixture (81,795 lines, 1.88 MB, 7 components): a full read is ~36ms, coalesced (2 reads per 5 Immediate lines). The prescribed CountOfLines-gate fix is DEFERRED, not taken: for C7 it is unsound (ResyncFromModule exists to catch edits made outside the surface, which a line-count check would miss - an in-line external edit preserves the count), and for C8 it trades the host-as-source-of-truth invariant for a saving that only appears on an 80k-line project during rapid Immediate use. The dominant cost is the text transfer, which cannot be skipped soundly; the avoidable part (the unindexed name walk) is single-digit ms even at 60 components. C13 is MEASURED and FIXED: on a twelve 500-line-module project, the first completion in each module cost a median 11ms against 2.6ms warm (4.2x, 161ms across the first walk, re-paid after every reseed) because the project index was rebuilt per module while the analyzer's 8-entry parse cache thrashed; one shared index per seed (WeakMap, the fingerprint pattern) brings cold to 3.5ms median and 1.3x, with the dispatcher's fingerprint index deliberately kept separate (its ignoreInvalidModules:false is load-bearing for freshness). All four engine suites green, freshness included. C10 and C17 are FIXED to their in-repo precedents: every splitter drag now defers Monaco layout to automaticLayout with one settling layout at the drag's end (livedrag.ts, shared by all three drags), and the Object Browser sorts its library once on arrival and debounces its search at 150ms, the find widget's wait. C12 is FIXED on both sides of the wire: the active-line hold call sites name the modules the hold change touched and PublishMarkersToSurface skips the open documents whose sets are provably unchanged, and the page's renderPanel returns on an unchanged key (every field the rows render, plus the severity filters) instead of destroying and rebuilding the list - setProblemCounts already had its own guard. C14's double flatten is FIXED via a one-slot version-keyed memo in documents.ts (wholeTextOf), shared by the bridge's fullText, loop-iterator sync, and smart Enter, cleared on model dispose; the 64,000 threshold's PRECONDITIONS ARE NOW MET: reconstruct-branch.mjs (the -Deep tier, its own session) types into a 60,022-char and a 68,002-char module through the real keyboard path, proves WHICH branch ran off the message tap (fullText present under, absent over), and holds workbook, surface and engine byte-identical through both - the reconstruction path's first test ever. Measured the same day: 88ms for the typing under the threshold against 68ms over it, so the reconstruction path is CHEAPER as well as correct - the fullText branch pays an O(document) build, serialize and parse per keystroke for nothing the ranges do not already carry. The call was made the same day: fullText is DROPPED (the owner, 2026-08-12). Every keystroke now crosses as ranges alone at every module size, the fullLength tripwire stays, the parity suites validate the one remaining path continuously, and reconstruct-branch.mjs pins both scales AND that the copy stays gone. The one-slot flatten memo the fullText build justified went with it |
| Duplication | done | B13 (five debug-door shapes: HostError/HostOk, PageReply, WaitForWorkspace, BenchReply, CrossToHost - the journal's crossing now feeds PerfCounters.Marshal), B14 (seven engine round trips through one AnswerFromEngine, with code actions' guard and the four quiet logs normalized), B15 (ten pending tables through one RequestTable, budgets kept per call), B16 (both ghost readers on one GhostWindowReader; the Watches walk gains the stage names), B17 (seven dialogs on one modal.ts with the Tab trap none of them had; five backdrop and five card CSS rules collapsed to per-dialog geometry), B18 (one ForEachRealComponent and ProjectReader.ReadSource everywhere; the projects route no longer drops a whole project over one bad component), B20 (was already done: the import's second read elided), B27 (window text/class reads live on Win32 beside their imports; DebugCapture's half was already done), and B29's cheap half (paneldocks' shadowing prune removed; the workspace TreeNode adoption stays open as its own change, as the finding prescribes). B25 is DONE: Find-XlideApi/Get-XlideApi in tools/harness/XlideApi.psm1 mirror the node client's discover()/open() - enumerate, prove liveness with /state, refuse to guess - and all ten hand-rolled discovery paths (a tenth had appeared since the audit counted) now go through it. B24 is DONE in its three parts: engine/test/harness.mjs (startEngine/notify/reporter, --exe now works for all four suites - positions and freshness meet the packaged executable for the first time), tools/FixtureDriver.ps1 (five generators become their VBA bodies plus one Invoke-FixtureBuild call, proven by regenerating TwinFixture end to end), and reporter()/scratchModule() in xlide-api.mjs (eight live suites print through one reporter; language-features keeps its tolerance verdict and module-sync its three-argument check DELIBERATELY - those are different sentences, not drifted copies) |
