...
internal static class AgentGuide
{
    ...
    [
        ...
        new("tests", "GET", "action, module, test, file, tags, outcomes",
            "The VBA test runner over EVERY open file: '@xlide-test' directives over zero-argument Subs in standard modules, the XlideAssert latched-failure protocol, results per test. No action lists, answering a files array - each file's own support state and test count - and a row per test carrying the file it came from. file= narrows any verb to one open file, which a module name cannot do on its own because it is only unique inside its own file. action=install writes/updates XlideAssert in one file with file= or in every file that holds tests without it; action=run runs everything everywhere, run&file= one file, run&module= that module wherever it is, run&test= one test; action=runFailed reruns what failed, narrowed by module= or file=; action=debug&test= runs one test untrapped so breakpoints and errors drop into the debugger. tags= and outcomes= narrow any run verb the way the pane's filters narrow its list: tags=smoke,fast means any of those tags, with the word untagged admitting tests that carry none, and outcomes= takes the pane's five chip groups - passed, failed, xfail, skipped, notRun - against each test's current standing, so run&tags=smoke&outcomes=failed is the pane's Run Displayed over a smoke filter and a failed-only chip. ranAt says when the last run finished.",
        ...
        new("changes", "GET", "action=text|diff|snapshot|accept|restore|reject, project=, module=, round=, which=, since=, label=, limit=, by=",
            ...
            + "in its turn: nothing this route does is out of its own reach. diff&since=accept "
            + "lines a module's accept-mark text against its LIVE text, and the bare listing "
            + "carries sinceAccept - the same comparison summed per module.",
        ...
        new("settings", "GET|POST",
            "blockLayout=? continueCommentOnNewline=? mirrorCommentSpacing=? insertOptionExplicit=? treeFollowsEditor=? formatIndentSize=? syncEngine=? designerSnap=? designerGridSize=? explorerView=? applyAttributesOnSave=?",
            "Reads the product settings, or writes the ones named. insertOptionExplicit (default true) makes every module xlide creates start with Option Explicit; restore never seeds, it recreates what was recorded. explorerView (tree|folders) is which layout the Explorer pane shows. applyAttributesOnSave (default true) makes a save write every module's attribute annotations into its attributes first.",
