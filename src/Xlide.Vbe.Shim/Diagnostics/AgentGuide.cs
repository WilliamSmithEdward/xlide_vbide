...
internal static class AgentGuide
{
    ...
    [
        ...
        new("settings", "GET|POST",
            "blockLayout=? continueCommentOnNewline=? mirrorCommentSpacing=? insertOptionExplicit=? treeFollowsEditor=? formatIndentSize=? syncEngine=? designerSnap=? designerGridSize=?",
            "Reads the product settings, or writes the ones named. insertOptionExplicit (default true) makes every module xlide creates start with Option Explicit; restore never seeds, it recreates what was recorded.",
