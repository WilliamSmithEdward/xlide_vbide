    ...
    private ApiServer.ApiReply AnswerApiRequest(ApiServer.ApiRequest request)
    {
        ...
        switch (request.Route)
        {
            ...
            case "immediate":
            {
                ...
                // THE PANEL'S WAY IN AS WELL, since a line that would not compile cut the page off
                // from the host while this route went on working (#30). One path, in
                // AddInSession.Immediate.cs, so what the door answers and what the panel shows
                // cannot drift apart again.
                var evaluated = EvaluateImmediateAway(text, ImmediateCaller.Door, WaitMilliseconds(request, 15000));
                return ApiServer.ApiReply.Json(System.Text.Json.JsonSerializer.Serialize(
                    new DebugImmediateReply(evaluated.Ran, evaluated.Text, evaluated.Failed),
                    DebugJsonContext.Default.DebugImmediateReply));
            }
