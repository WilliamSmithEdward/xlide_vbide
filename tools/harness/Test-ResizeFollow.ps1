...
# workspace resized in silence. Run tools\dev.ps1 -KeepOpen first, then this. Expected:
...
# Resizes the VBE frame twice per state and compares the overlay rect to the frame client.
...
function Report([string]$title) {
    ...
    $match = if ($ow -eq $cw -and $oh -eq $ch) { 'MATCH' } else { 'MISMATCH' }
    Write-Output "$title -- client ${cw}x${ch} overlay ${ow}x${oh} -> $match"
}
