...
export function instructionsFor(state: ApiState): string {
  ...
  return [
    ...
    "",
    "What is here, by job (`agent/routes` has every argument):",
    "",
    "  Read the project   projects, project, documents, module, outline, problems, attributes,",
    "                     tests, changes, breakpoints, locals, watches, state",
    "  Change code        module (POST, body = the module's whole text), component (add, remove,",
    "                     rename), attributes?action=apply, sync (import or export a folder),",
    "                     settings",
    "  Run and debug      compile, command (run, save, stepInto and the rest), caret, breakpoint,",
    "                     immediate, tests?action=run",
    "  Undo and review    changes: text, diff, snapshot, accept, restore, reject",
    "  Look and drive     ui, act, capture (a PNG), dialogs, dismiss, assert, layout, windows",
    "  Health             doctor, log, journal, stats, analyzer, analysis, model, sessions",
    "",
    ...
    "- Undo is the change log. `changes?action=restore&round=N` puts the project back to the",
    "  state after round N, or one module with `module=`; `action=reject` returns to the accept",
    "  mark. A restore is itself a round under your name, so it can be restored away too.",
    "- Give the analyzer its moment. A write is analysed asynchronously and `problems` answers",
    "  the last COMPLETED pass, so read it straight after a write and you may read the pass",
    "  before yours. Wait instead: `GET assert?that=problemFree&timeoutMs=8000` answers the",
    ...
    "",
    "Write for the second time it runs:",
    "",
    "- Ask what it is for before you build it. Handed a data set, find out whether this is a",
    "  one-off or something that runs again next month against more rows and fewer columns.",
    "  Then say which of the two you built.",
    "- Read the shape you find rather than the shape you were shown. Size ranges from the sheet",
    "  itself - the last used row, a ListObject, CurrentRegion - so that adding a row does not",
    "  quietly leave it out. A total that names row 47 is wrong the first time someone inserts",
    "  one.",
    "- Write formulas into the cells, not the answers, wherever the developer will want the",
    "  number to follow the data. A `=SUMIFS(...)` left in the sheet keeps being right; a pasted",
    "  value stops being right the moment anything changes.",
    "- Leave it readable. Fit the columns so numbers do not land as ###, give a table its header",
    "  row and a blank row or column against whatever it sits next to, and set number formats",
    "  for money, dates and percentages rather than leaving raw doubles on the sheet.",
    "",
